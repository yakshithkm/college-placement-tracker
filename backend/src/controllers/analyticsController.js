...
const calculateReadinessScore = async (studentId) => {
  const [student, projects, certs, aptitude, interviews, applications, skills] = await Promise.all([
  ...
  // Aptitude score (20%)
  const aptPct = parseFloat(aptitude.rows[0].avg_pct || 0);
  const aptCount = parseInt(aptitude.rows[0].count);
  const aptitudeScore = aptCount > 0 ? aptPct : 0;
  ...
  if (aptCount < 3) recommendations.push({ type: 'aptitude', message: 'Practice more aptitude tests to improve your scores' });
  ...
  const snapshot = {
    ...
    resumeScore: parseFloat(resumeScore) || 0,
...
exports.getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const [projects, certs, applications, aptitude, interviews, latestSnapshot] = await Promise.all([
