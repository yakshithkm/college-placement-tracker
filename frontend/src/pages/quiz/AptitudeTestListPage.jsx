  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <a href="/aptitude-tests/history" onClick={(e) => { e.preventDefault(); navigate('/aptitude-tests/history'); }} className="btn btn-secondary">📊 My History &amp; Analytics</a>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 24 }}>
        ...
        {categories.map(c => (
          ...
          >
            {c.icon} {c.name} <span style={{ opacity: 0.8 }}>({c.test_count})</span>
          </button>
        ))}
      </div>
          ...
          {tests.map(t => (
            <div key={t.id} className="card" style={{ borderTop: `3px solid ${t.category_color || '#2563EB'}` }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                <div style={{ fontSize: 26 }}>{t.category_icon || '📝'}</div>
                <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                <span className="badge badge-blue">❓ {t.actual_question_count} questions</span>
                <span className="badge badge-purple">🏆 {t.total_marks} marks</span>
                {t.timer_enabled && <span className="badge badge-yellow">⏱ {t.duration_minutes} min</span>}
